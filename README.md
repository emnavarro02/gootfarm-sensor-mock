...
For moisture, the sensor node emulator will generate a random number between 0 - 100.
For temperature, the sensor node emulator generates a random number between 12 - 42.
...
Alternatively it is possible to specify the type and value of a measurement by using the switches `--type` and `--value`. In this mode, the script will execute only once.
...
| -b, --broker_address     | The MQTT broker IP address                                       | '127.0.0.1'| String |
| -p, --broker_port     | The MQTT broker port                                             | 1883       | int    | 
| -T, --type      | Type of measurement to be sent to the server                     | None       | String | 
| -v, --value     | Value of measurement to be sent to the server                     | None       | int    | 
...
* Launches the sensor node emulator in _run-once_ mode, and send a measurement to the listener:
```{powershell}
